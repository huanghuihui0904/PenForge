...
class Episode(BaseModel, Generic[AnyProposal]):
    ...
    def format(self):
        ...
        # Status string
        status = self.result.status if self.result else 'did_not_finish'

        # Output string (multiline indented if needed)
        if self.result.status == "success":
            result_output = str(self.result)
            result_output = "\n" + indent(result_output) if "\n" in result_output else result_output
        elif self.result.status == "error":
            result_output=f"Error: {self.result.error}\n"
        elif self.result.status == "interrupted_by_human":
            result_output= f"Feedback: {self.result.feedback}\n"


        # Single JSON object
        json_record = {
            "Action": f"{self.action.use_tool}",
            "Status": f"{status}",
            "Output": f"{result_output}"
        }

        # Append to thoughts.jsonl (1 JSON object per line)
        with open("/logs/thoughts.jsonl", "a", encoding="utf-8") as f:
...
class EpisodicActionHistory(BaseModel, Generic[AnyProposal]):
    ...
    async def handle_compression(
        ...
        compress_instruction = (
            ...
            "\n**Important:** If any step involves token, cookie, or session usage, do NOT include their actual values.\n"
